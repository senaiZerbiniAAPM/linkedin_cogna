...
Versão SEM busca de e-mail — mantém persistência (SQLite), export e consulta.
...
# ---------------- Residence extraction heuristics (new) ----------------
# Brazil state names & abbreviations (lowercase / typical forms) to help validate matches.
BRAZIL_STATE_NAMES = [
...
# common separators used in LinkedIn/snippets (commas, bullets, bars, dashes)
_LOCATION_SEP = r"(?:,|\u2022|\u00B7|\||–|—|-)"

def extract_residence_from_description(text: Optional[str]) -> Optional[str]:
    """
    Heurística para extrair **local de moradia** a partir do texto (description/snippet).
    Retorna string curta tipo "Cidade, Estado, País" ou None se não achar algo plausível.
    """
    if not text or not isinstance(text, str):
        return None
    txt = text.strip()
    # normalize some separators to comma for easier regex
    norm = re.sub(r"[\u2022\u00B7\|–—\-]", ",", txt)
    # remove excessive whitespace
    norm = re.sub(r"\s+", " ", norm)

    # 1) procurar padrão no início: "Cidade, Estado, Brasil" ou "Cidade, Estado"
    ...
    # 2) procurar em qualquer parte: "Cidade, Estado" onde Estado seja nome/abbr BR
    ...
    # 3) LinkedIn sometimes prints "City, Country" only: try pattern city,country and validate country common names
    ...
    # 4) reject segments that look like job/education statements (presence of keywords)
    ...
    # 5) fallback: try to extract a short capitalized phrase that looks like a city name (risky)
    m4 = re.search(r"\b([A-ZÀ-Ý][a-zà-ÿ]+(?:\s+[A-ZÀ-Ý][a-zà-ÿ]+){0,2})\b", norm)
    if m4:
        candidate = m4.group(1).strip()
        # only return if short (avoid returning 'Engenharia de Sistemas' etc.)
...
def init_db():
    """Cria a tabela se não existir (SEM coluna 'email')."""
...
def save_profiles_to_db(df: pd.DataFrame, conn: sqlite3.Connection) -> Tuple[int, int]:
    ...
    Evita duplicados verificando profile_link já existente.
...
def query_profiles(conn: sqlite3.Connection, location: str = "", competence: str = "") -> pd.DataFrame:
    """
    Consulta o DB por localização e competência (LIKE sobre local_desc).
    """
    cur = conn.cursor()
...
# --- NOVA normalize_item_for_table (prioriza residência) ---
def normalize_item_for_table(item: Dict) -> Dict[str, Optional[str]]:
    """
    Normaliza um item (dicionário) retornado como 'organic' em campos:
    nome, Link de perfil, Local e descrição.
    Agora prioriza extração de local de moradia (residência).
    """
    title = (item.get("title") or item.get("job_title") or "") if isinstance(item, dict) else ""
    # tentar campos estruturados comuns que scrapers podem retornar
    link = item.get("link") or item.get("url") or item.get("source_url") or item.get("final_url") or None
    # campos possíveis de localização estruturada
    ...
    # descrição/snippet (backup)
    description = (item.get("description") or item.get("snippet") or "") if isinstance(item, dict) else ""
    # heurística para extrair local de moradia a partir da descrição
    extracted_residence = extract_residence_from_description(description)

    # choose best local: prefer structured loc_candidate if it *looks like* a residence (contains comma or country/state)
    chosen_local = None
    if loc_candidate:
        # quick check: if contains comma and a known state/country token return it
        if "," in loc_candidate or any(s.lower() in loc_candidate.lower() for s in ["brasil","brazil","portugal"]):
            chosen_local = loc_candidate
        else:
            # try running normalization on it too (attempt to find e.g. "São Paulo" or "Lisboa")
            maybe = extract_residence_from_description(loc_candidate)
            chosen_local = maybe or loc_candidate

    # if structured not present or not convincing, use extracted from description
    if not chosen_local and extracted_residence:
        chosen_local = extracted_residence

    # Final fallback: if none found, keep the original description (so you don't lose context)
...
# botão com key único para montar tabela a partir do JSON
...
if count == 0:
    ...
    # === EXPORT: agora exporta TODO o DB (CSV) e também XLSX ===
    ...
    # botões salvar e reset (com keys únicas)
...
with btns[1]:
    # botão Consulta: abre a "outra tela" para consulta por Localização e Competência
...
# Consulta: tela/modal (renderização condicional)
...
# mostrar JSON cru se disponível
if show_raw:
